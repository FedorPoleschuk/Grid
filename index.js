module.exports = function (dest, options) {
  try {
    let root = __dirname;
    let fs = require("fs");
    let defaults = require("./components/defaults/settings.js");
    ...
    let patterns = {};
    patterns.mixin = fs.readFileSync(root + "/components/patterns/mixin");
    patterns.clearfix = fs.readFileSync(root + "/components/patterns/clearfix");
    patterns.reset = fs.readFileSync(root + "/components/patterns/reset");
    patterns.fromTo = fs.readFileSync(root + "/components/patterns/fromTo");

    let patternDebug = fs.readFileSync(root + "/components/patterns/debug");
    let sizeCalling = ["(1)"];
    ...
    patterns.debug = patternDebug.toString().replace("{{sizes}}", sizeCalling.join("\n"), "g").replace("{{mixinNames.container}}", options.mixinNames.container, "g").replace("{{mixinNames.row-flex}}", options.mixinNames.row, "g").replace("{{mixinNames.col}}", options.mixinNames.column, "g");
